...
- [ ] **A1. 数据下载与统一格式**
  ...
  - 统一字段：`id, dataset, split, question, answer, supporting_facts(optional)`。
...
- [ ] **C1. 计算真实保留值 $r_k^*$**
  - 定义：基于全轨迹离线回放，枚举“停/继续”动作，得到最优停止点。
  - 输出：`artifacts/oracle/{dataset}/{split}_oracle_labels.jsonl`。
  - 验收：可回放复现 Oracle 决策，且决策路径无非法状态。

- [ ] **C2. 绘制 Oracle Pareto Frontier**
  - 横轴：Avg Cost（或 Avg Steps）；纵轴：F1。
...
- [ ] **D1. 阶段报告**
  ...
  - 必含内容：数据统计、缓存完整性、特征分布、Oracle 前沿图、失败样例。
  - 验收：报告可独立阅读复现阶段结论。
...
*   预实验中预测绝对 `F1` 效果差。改为预测**期望边际增益 (Expected Marginal Gain)**：$y = \mathbb{E}[F_{k+1}] - F_k$ 或者直接预测距离保留值的差值 $\Delta r = r_k^* - \hat{Q}(s_k)$。
*   **分类建模 (Alternative)**：如果回归任务依然困难（$R^2 < 0.3$），直接将其转化为**二分类任务**（是否满足 Oracle 停止条件：继续 vs 停止），使用 Focal Loss 解决数据不平衡问题。
...
1.  **风险：Probe 怎么训都不准 (回归 $R^2 < 0.2$)**
    *   *应对方案*：放弃连续值的 Reservation Value 回归预测。直接把 Oracle 给出的停止节点作为二分类标签（`1=Stop, 0=Continue`），训练一个以隐藏状态为输入的二分类器。虽然稍微偏离了纯粹的 Weitzman 解析解，但可以通过“Neural Approximation”的说法圆回来，且工程落地效果往往更好。
