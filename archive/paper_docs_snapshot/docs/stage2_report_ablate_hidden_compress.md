...
```bash
# 默认 d=64：与仓库当前主结果一致（见 docs/stage2_report.md）
...
Shallow-Only：`docs/stage2_report_shallow.md`（HotpotQA 0.4829，MuSiQue 0.1394，2Wiki 0.3552）。
...
**论文/工程建议**：若只选一个折中，可优先考虑 **d=256** 作为「放宽瓶颈」的消融点；2Wiki 仍建议保持 **d=64** 或单独调参（如 `fuse_dim`、正则或按数据集选 d）。详细分项报告与 CSV/PNG：`docs/stage2_report_ablate_compress_dim256.md`、`docs/stage2_report_ablate_compress_dim512.md`。