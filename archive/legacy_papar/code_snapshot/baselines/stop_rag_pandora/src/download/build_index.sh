...
if [ "$RETRIEVER" = "contriever" ]; then
    for ds in hotpotqa 2wikimultihopqa musique; do
        uv run --no-sync python -m pipeline.contriever.passage_embedder \
            --passages data/corpus/$ds/passages/corpus.jsonl \
            --output_dir data/corpus/$ds/contriever_embeddings

        uv run --no-sync python -m pipeline.contriever.passage_retriever \
            --passages data/corpus/$ds/passages/corpus.jsonl \
    ...
    for ds in hotpotqa 2wikimultihopqa musique; do
        uv run --no-sync python -m pipeline.bm25.bm25_retriever \
            --passages data/corpus/$ds/passages/corpus.jsonl \
