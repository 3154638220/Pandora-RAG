...
def analyze_and_split_data(
    file_path, train_ratio=0.8, eval_ratio=0.1, random_state=42, output_dir=None, target_label="f1"
):
    ...
    print(
        f"Train set: {len(train_df)} samples ({len(train_df)/len(df)*100:.1f}%) - avg {target_label}: {train_avg_label:.6f}"
    )
    print(
        f"Eval set: {len(eval_df)} samples ({len(eval_df)/len(df)*100:.1f}%) - avg {target_label}: {eval_avg_label:.6f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--target-label", type=str, default="f1", choices=["prob", "em", "f1"], help="Target label for splitting"
    )
