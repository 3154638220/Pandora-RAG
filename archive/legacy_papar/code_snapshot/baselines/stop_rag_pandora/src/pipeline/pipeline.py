...
def run_batch(
    ...
    while questions:
        ...
        inter_answers = intermediate_answer_generator.batch_generate_intermediate_answers(
            search_queries, batch_selected_docs
        )
        ...
        stop_decisions = stop_decider.batch_decide(
            questions,
            traces,
            fields,
            log_trace=log_trace,
    ...
    final_predictions = (
        final_answer_generator.batch_generate_final_answers(final_questions, final_batch_history, fields)
        if final_questions
        else []
    )
    ...
    em_list, precision_list, recall_list, f1_list = compute_retrieval_metrics(
        final_questions, final_batch_history, stop_logs, fields
    )
    ...
    if traces_path:
        all_ans_em_list, all_ans_f1_list, all_ans_acc_list = compute_all_answer_metrics(
            final_questions, final_predictions, fields
        )
    ...
    return final_questions, final_batch_history, final_predictions, metrics, stop_logs


def summarize_stop_logs(stop_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
    stop_iters = [int(log["stop_iter"]) for log in stop_logs if "stop_iter" in log]
    dist = Counter(stop_iters)
    return {
        "n": len(stop_iters),
        "avg_steps": float(sum(stop_iters) / len(stop_iters)) if stop_iters else 0.0,
...
def parse_args():
    ...
    retriever_group.add_argument("--contriever-model-path", type=str, default="facebook/contriever-msmarco", help="Local path or model ID for Contriever")
    # bm25
    ...
    vllm_group.add_argument(
        ...
        help="Model ID or local path for vLLM (default: STOP_RAG_VLLM_MODEL, else Pandora models/Meta-Llama-3.1-8B-Instruct, else Hub id)",
    )
    vllm_group.add_argument("--vllm-tp-size", type=int, default=4, help="Tensor parallel size for vLLM")
    ...
    vllm_group.add_argument(
        "--vllm-enforce-eager",
        action="store_true",
        help="Disable CUDA graph capture (slower but avoids OOM during init on 24GB×TP with long max_model_len)",
    )
    ...
    reranker_group.add_argument("--reranker-max-length", type=int, default=768, help="Maximum length for reranker input")
    ...
    stop_decider_group.add_argument("--sd-provider", type=str, default="vllm", choices=["vllm", "openai", "nostop", "multihead"], help="Provider for stop decider")
    stop_decider_group.add_argument("--sd-checkpoint-path", type=str, default=None, help="Checkpoint path for multihead stop decider")
    stop_decider_group.add_argument("--sd-threshold", type=float, default=0.0, help="Decision threshold for multihead stop decider")
    stop_decider_group.add_argument("--sd-max-length", type=int, default=2048, help="Max encoder length for multihead stop decider")
    stop_decider_group.add_argument("--sd-bf16", action="store_true", help="Use bf16 for multihead stop decider")
    ...
    main_group.add_argument("--batch-size", type=int, default=1024, help="Batch size for processing questions")
...
def main(args: argparse.Namespace):
    ...
    if args.retriever_type == "contriever":
        ...
        retriever = Retriever(
            ...
            model_path=args.contriever_model_path,
        )
    ...
    vllm_agent = None
    if "vllm" in [args.qg_provider, args.iag_provider, args.sd_provider]:
        vllm_agent = LLM(
    ...
    openai_config = None
    if "openai" in [args.qg_provider, args.iag_provider, args.sd_provider]:
    ...
    stop_decider = StopDecider(
        ...
        checkpoint_path=args.sd_checkpoint_path,
        threshold=args.sd_threshold,
        max_length=args.sd_max_length,
        bf16=args.sd_bf16,
    )
    ...
    for i in tqdm(range(0, len(questions), args.batch_size)):
        ...
        final_questions, final_batch_history, final_predictions, batch_metrics, batch_stop_logs = run_batch(
            retriever=retriever,
            query_generator=query_generator,
            reranker=reranker,
            intermediate_answer_generator=intermediate_answer_generator,
            final_answer_generator=intermediate_answer_generator,
        ...
        all_stop_logs.extend(batch_stop_logs)
    ...
    if args.output_path:
        output_data = {
            "metrics": all_metrics,
            "stopping": summarize_stop_logs(all_stop_logs),
            "predictions": [],
        }
        _fields = DATASET_FIELDS[args.dataset]

        for q, h, p in zip(all_final_questions, all_final_batch_history, all_final_predictions):
            output_data["predictions"].append(
                {
                    "id": q[_fields["id"]],
                    "question": q[_fields["question"]],
                    "gold_answer": q.get(_fields["answer"], ""),
