...
class QueryGenerator:
    def __init__(
        self, llm, max_gen_length=200, temperature=0.7, top_p=0.9, provider="vllm", retriever_type="contriever"
    ):
    ...
    def batch_generate(
        self, questions: List[Dict[str, Any]], traces: List[str], fields: Dict[str, str]
        ...
        if self.retriever_type == "contriever":
            ...
            prompts = [
                gen_bm25_query_prompt(question[fields["question"]], trace) for question, trace in zip(questions, traces)
            ]
        else:
