...
def parse_args():
    ...
    parser.add_argument("--model_type", type=str, default="e5-base-v2", choices=list(ModelTypes.keys()), help="Embedding Model")
    parser.add_argument("--model_name_or_path", type=str, default=None, help="Embedding model checkpoint")
    parser.add_argument("--save_or_load_index", action="store_true")
    parser.add_argument("--embeddings", type=str, required=True, help="Document embedding path")
...
def test(opt: argparse.Namespace):
    retriever = Retriever(opt.passages, opt.embeddings, model_type="contriever")
