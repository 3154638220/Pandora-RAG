...
class HotpotQADataset(MultiHopDataset):
    ...
    def get_chunks(self, sample: dict) -> list[ChunkInfo]:
        chunks = []
        for idx, (title, context) in enumerate(zip(sample["context"]["title"], sample["context"]["sentences"])):
...
class WikiMQADataset(MultiHopDataset):
    ...
    def get_decomposition(self, sample: dict):
        ...
        if len(sample["supporting_facts"]) != len(sample["evidences"]):
            return []
        for supporting_fact, evidence in zip(sample["supporting_facts"], sample["evidences"]):
...
class MuSiQueDataset(MultiHopDataset):
    ...
    def get_chunks(self, sample: dict) -> list[ChunkInfo]:
        ...
        for paragraph in sample["paragraphs"]:
            ...
            chunk = paragraph["paragraph_text"]
...
def get_dataset(dataset: str, input_path: str) -> MultiHopDataset:
    if dataset == "hotpotQA":
        return HotpotQADataset(input_path)
    elif dataset == "2WikiMQA":
        return WikiMQADataset(input_path)
    elif "musique" in dataset:
        return MuSiQueDataset(input_path)
    else:
