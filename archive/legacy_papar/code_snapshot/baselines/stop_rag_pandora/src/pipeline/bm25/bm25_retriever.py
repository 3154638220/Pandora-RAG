...
class BM25Retriever(object):
    ...
    def _load_index(self):
        self.bm25 = bm25s.BM25.load(self.index_path, load_corpus=False, mmap=self.use_mmap)
    ...
    def search(self, query: Union[str, List[str]], top_k: int = 10):
        ...
        docs, scores = self.bm25.retrieve(query_tokens, corpus=self.passages, k=top_k, show_progress=False)
...
def parse_args():
    ...
    parser.add_argument("--passages", type=str, required=True, help="document file path")
    parser.add_argument("--model_type", type=str, default="bm25s", help="Retrieval model type (bm25s)")
    parser.add_argument("--model_name_or_path", type=str, default=None, help="Not used for BM25S but kept for compatibility")
    parser.add_argument("--index_path_dir", type=str, default=None, help="Directory to save or load the BM25S index")
    parser.add_argument("--save_or_load_index", action="store_true")
    parser.add_argument("--embeddings", type=str, default=None, help="Not used for BM25S but kept for compatibility")
    ...
    parser.add_argument("--method", type=str, default="lucene", choices=["lucene", "robertson", "atire", "bm25l", "bm25+"], help="BM25 variant method")
    parser.add_argument("--use_mmap", action="store_true", help="Use memory mapping for large indices")
