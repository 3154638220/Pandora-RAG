...
- `[plan.md](plan.md)` 仍把「答案 token 级置信度 / 自评」列为高价值方向；**实现上** Stage1 已在每步写入 `answer_logprob`（当前来自 `pretest/utils/llm_client.py` 对**首条采样答案**的平均 token logprob）与 `self_eval_score`（额外一次自评调用）。
...
Probe 的公平对照是 **Deployable-GW**（测试时用可部署代理停止、再报真实 F1），而不是在 test 上用真 F1 停表的 Original-GW。主线达标后，不必以「追平 Original-GW」为主优化目标，详见 `[plan.md](plan.md)` 中 Deployable-GW 与 Oracle 口径说明。
...
新特征接入后的标准验收链路与此前相同：诊断 → `run_stage2` → 与当前 Probe / Deployable-GW 及 `[docs/stage2_report.md](stage2_report.md)` 中的表对齐。
