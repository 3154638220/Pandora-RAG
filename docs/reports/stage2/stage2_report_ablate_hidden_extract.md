...
对应 `docs/plans/plan-04-10.md` §三.4。分两组：**§1** 统一默认超参 + **F1 回归**（与仓库默认 CLI 一致）；**§2** **per-dataset 最优超参** + **二分类**（与 Continue/Stop 标签一致，适合作为「主线分类 Probe」下的 Hidden 策略对照）。
...
**小结：** 在 **二分类 + 最优超参** 下，**2Wiki** 上 **`--hidden-branch-residual` 相对 `last_token` +0.0275 F1**；HotpotQA 残差与基线持平；MuSiQue 仍以 `last_token` 最优。注意：同超参下 **F1 回归头** 的 2Wiki test 仍可达约 **0.383**（见 `docs/plans/plan-04-10.md` §一），与二分类数值不可直接横比。
