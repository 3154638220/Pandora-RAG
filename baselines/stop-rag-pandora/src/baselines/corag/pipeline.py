...
def run_batch(
    ...
    if not fag_disable:
        predictions = generator.batch_generate_final_answers(final_questions, final_batch_history, final_traces, fields)
    else:
        predictions = [""] * len(final_questions)

    if log_trace:
        print("\n========== Final Questions and History ==========")
        for question, history, prediction in zip(final_questions, final_batch_history, predictions):
    ...
    em_list, precision_list, recall_list, f1_list = compute_retrieval_metrics(final_questions, final_batch_history, [], fields)

    ans_em_list, ans_f1_list, ans_acc_list = compute_answer_metrics(final_questions, predictions, fields)

    if traces_path:
        all_ans_em_list, all_ans_f1_list, all_ans_acc_list = compute_all_answer_metrics(
            final_questions, predictions, fields
        )
    ...
    if log_trace and final_questions:
        print("\n========== Final Results ==========")
        for q, hist, trace, pred in zip(final_questions, final_batch_history, final_traces, predictions):
            print(f"\nQID: {q[fields['id']]}")
    ...
    return final_questions, final_batch_history, predictions, metrics
...
def parse_args():
    ...
    retriever_group.add_argument(
        "--contriever-model-path",
        type=str,
        default="facebook/contriever-msmarco",
        help="Local path or model ID for Contriever (match src.pipeline.pipeline)",
    )
    # bm25
    ...
    stop_decider_group.add_argument("--sd-provider", type=str, default="nostop", choices=["vllm", "nostop", "multihead"], help="Provider for stop decider")
...
def main(args: argparse.Namespace):
    ...
    stop_decider = StopDecider(
        llm=llm if args.sd_provider == "vllm" else None,
