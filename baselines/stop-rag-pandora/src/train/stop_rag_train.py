...
try:
    import wandb
except ImportError:  # pragma: no cover - optional dependency for local reproduction
    class _WandbStub:
        run = None

        @staticmethod
        def init(*args, **kwargs):
            raise RuntimeError("wandb is not installed. Re-run with --disable-wandb or install wandb.")
...
class MultiheadStopDecisionDataset(Dataset):
    def __init__(self, filepath, tokenizer, max_iterations=5, max_length=2048, target_label="prob"):
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            self.data = [
                trace
                for trace in self.full_data
                if trace["iter_cnt"] < max_iterations
                and max([trace[f"{target_label}"]] + trace[f"cont_{target_label}"] + [trace[f"last_{target_label}"]])
                > 0.0
            ]
    ...
    def _trace_to_encoding(self, trace, add_labels=True):
        ...
        text = sep_token.join([trace["question"]] + [f"{doc['title']}: {doc['text']}" for doc in trace["history"]])
...
class MultiheadModel(PreTrainedModel):
    ...
    def __init__(self, config, encoder_kwargs={}, dtype=None, inference_mode=False):
        ...
        if hasattr(config, "encoder_lora_config") and config.encoder_lora_config is not None:
            if (
                encoder_quantization_config is not None
                and (encoder_quantization_config.load_in_4bit or encoder_quantization_config.load_in_8bit)
                and not inference_mode
            ):
                self.encoder = prepare_model_for_kbit_training(
                    self.encoder, use_gradient_checkpointing=config.use_gradient_checkpointing
                )
    ...
    def load_state_dict(self, state_dict, strict=True, assign=False):
        ...
        state_dict_classifier1 = {k.split(".", 1)[-1]: v for k, v in state_dict.items() if k.startswith("classifier1.")}
        ...
        state_dict_classifier2 = {k.split(".", 1)[-1]: v for k, v in state_dict.items() if k.startswith("classifier2.")}
        ...
        return _IncompatibleKeys(
            missing_encoder + missing_classifier1 + missing_classifier2,
...
class MultiheadTrainer(Trainer):
    ...
    def _compute_tdlambda_labels(self, model, inputs):
        ...
        nstep_labels, _ = torch.cat([cont_labels, bs_labels_head1, bs_labels_head2], dim=1).max(
            dim=1
        )  # (batch_size, n)
...
def parse_args():
    ...
    parser.add_argument("--target-label", type=str, default="f1", choices=["prob", "em", "f1", "acc"], help="Target label for training")
    ...
    parser.add_argument("--max-grad-norm", type=float, default=50.0, help="Max Gradient Norm")
    ...
    parser.add_argument("--save-total-limit", type=int, default=10, help="Total Number of Saved Checkpoints")
    parser.add_argument("--logging-steps", type=int, default=10, help="Logging Steps")
    parser.add_argument("--deepspeed-config", type=str, default="src/train/deepspeed_config.json", help="DeepSpeed Configuration File Path")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a saved checkpoint directory (e.g. outputs/.../checkpoint-200)",
    )
