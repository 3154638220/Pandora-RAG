...
表示到第 k 步为止的历史信息。停止时刻 \tau\in0,\ldots,K 必须是关于过滤族 (\mathcal{F}*k)*{k=0}^K 的 stopping time，即是否在第 k 步停止只能依赖当前及过去信息。
...
\pi^*\in\arg\max_\pi
\mathbb{E}*\pi\left[
Q(s*{\tau_\pi})-\sum_{j=1}^{\tau_\pi}c_j
\right].
\tag{2}
$$
...
$$
V_k^*(s_k)=
\sup_{\tau\in\mathcal{T}*k}
\mathbb{E}\left[
Q(s*\tau)-\sum_{j=k+1}^{\tau}c_j
\mid \mathcal{F}_k
\right],
\tag{3}
$$
...
\max\left
Q(s_k),
\mathbb{E}\left[V_{k+1}^*(s_{k+1})\mid\mathcal{F}*k\right]-c*{k+1}
\right.
\tag{5}
$$
...
\widetilde V_k^i=\max\leftQ_k^i,\widetilde V_{k+1}^i-c_{k+1}^i\right.
\tag{9}
$$
...
$$
\mathcal{L}*{\mathrm{cls}}(\theta)=
\mathbb{E}\left[
w(m_k)
\ell*{\mathrm{focal}}\left(p_\theta(k),y_k\right)
\right],
\tag{16}
$$
...
**定理 1.** 若 H_0 成立，且每个 \lambda_n 都是 \mathcal{G}*{n-1}-可测并取值于 [0,1]，则 (E_n)*{n\ge0} 是非负超鞅。因而对任意 \delta\in(0,1)，
...
$$
\mathbb{E}[M_n\mid\mathcal{G}*{n-1}]
=1-\lambda_n+\lambda_n
\frac{\mathbb{E}[e_n\mid\mathcal{G}*{n-1}]}{\alpha}
\le1.
\tag{25}
$$
...
$$
\mathbb{E}[E_n\mid\mathcal{G}*{n-1}]
=E*{n-1}\mathbb{E}[M_n\mid\mathcal{G}*{n-1}]
\le E*{n-1}.
\tag{26}
$$
