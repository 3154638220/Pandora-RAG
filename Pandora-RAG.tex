...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{paper/figs/stage3_final_pareto_all.pdf}
  \caption{Answer F1 versus average retrieval steps (upper-left is preferable). Oracle is a structural upper bound from rolled-out trajectories, not a deployable baseline, and is shown as a hollow star; the dashed line connects non-dominated points. Insets magnify the Probe / Probe+E-value / Probe+CP / Global-Weitzman region where differences are small.}
  \label{fig:pareto}
\end{figure}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{paper/figs/evalue_hotpotqa_triple.pdf}
  \caption{Representative online risk-evidence plots for \hotpot{} at $\alpha=0.1$ and $\gamma=0.5$. \emph{Left:} E-wealth under no shift, remaining well below the alarm boundary at $1/\alpha=10$ for most of the stream. \emph{Middle:} the same monitor under sudden shift, where wealth rises to the cap once harder examples are concentrated late in the stream. \emph{Right:} cumulative error (sudden shift): the main panel plots the excess cumulative error relative to Probe so that small differences are visible; the inset shows raw cumulative error on the late segment of the stream. The E-value layer is an online evidence process on the stopped stream, not a mechanism that forces the empirical error curve below $\alpha$.}
...
\begin{figure}[htbp]
  \centering
    \includegraphics[width=\linewidth]{paper/figs/stop_rag_pareto_1x3.pdf}
